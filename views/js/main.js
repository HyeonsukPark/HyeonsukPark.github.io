...
var resizePizzas = function(size) {
    ...
    function changePizzaSizes(size) {
        ...
        var dx = determineDx(a[i], size);
        var newwidth = (a[i].offsetWidth + dx) + 'px';
        for (i; i < a.length; i++) {
...
/* variables outside for-loop. it doesn't create all of pizzas anymore. */
...
function logAverageFrame(times) { // times is the array of User Timing measurements from updatePositions()
    ...
    var sum = 0;
    var i = numberOfEntries - 1;
    // var i is outside for-loop. 
    for (i; i > numberOfEntries - 11; i--) {
...
function updatePositions() {
    ...
    /* var phase inside for-loop iterates. It should be outside for-loop. */
    for (i; i < pizzaLen; i++) {
        phase = Math.sin((scroll / 1250) + (i % 5));
